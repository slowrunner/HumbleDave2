...
MPU9250Driver::MPU9250Driver() : Node("mpu9250publisher")
{
  ...
  // MOD: slowrunner to use sensor_data qos (best_effort instead of reliable)
  auto qos = rclcpp::QoS(rclcpp::KeepLast(10), rmw_qos_profile_sensor_data);
  publisher_ = this->create_publisher<sensor_msgs::msg::Imu>("/imu_sensor/imu", qos );  // keep last 10, best effort
