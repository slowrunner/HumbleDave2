...
"""
        ...
        Implements ROS 2 Humble autonomous mobile robot (HumbleDave2) using  
...
class DaveNode(Node):
    ...
    def __init__(self):
        ...
        printMsg = '---- HumbleDave2 node started - Undock:{:d} mA  Ready:{:.1f}v  Dock:{:.1f}v'.format(int(UNDOCK_AT_MILLIAMPS), GET_READY_TO_DOCK_AT_VOLTS,  DOCK_AT_VOLTS )
            ...
            if (self.state == "init"):       # Handle startup - wait for subcriptions, then figure out if docked or undocked
                ...
                if self.dock_svc_future.done() == True:  # print("self.undock_svc_future:", self.undock_svc_future.__dict__)
                    ...
                    if self.battery_state.charging == True:
                        ...
                        if (playtimeDurationInHours > 0.1):
                            printMsg = "---- HumbleDave2 ROS 2 Docking {} : success at battery {:.1f}v after {:.1f} h playtime ".format(chargeCycles, self.docked_at_Vbatt, playtimeDurationInHours)
                            self.lifeLog.info(printMsg)
                            dtstr = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            print(dtstr, printMsg)
                        ...
                        if (playtimeDurationInHours > 0.1):
                            printMsg = "---- HumbleDave2 ROS 2 Docking : failure at battery {:.1f}v after {:.1f} h playtime ".format(self.docked_at_Vbatt, playtimeDurationInHours)
                            self.lifeLog.info(printMsg)
                            dtstr = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            print(dtstr, printMsg)
                        self.prior_state = self.state
                ...
                if (self.battery_state.milliamps > UNDOCK_AT_MILLIAMPS):
                    ...
                    if (chargeDurationInHours > 0.1):
                        printMsg = "---- HumbleDave2 ROS 2 Undocking, Charge Current {:d} mA   {:.1f}v after {:.1f} h charging".format(int(abs(self.battery_state.milliamps)),self.battery_state.volts, chargeDurationInHours)
                        self.lifeLog.info(printMsg)
                        dtstr = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        if DEBUG:
                            print(dtstr,printMsg)
