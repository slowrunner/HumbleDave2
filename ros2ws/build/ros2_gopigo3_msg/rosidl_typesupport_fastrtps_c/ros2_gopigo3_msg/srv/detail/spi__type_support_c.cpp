  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = ros2_gopigo3_msg__srv__SPI_Request;
    is_plain =
      (
      offsetof(DataType, data_out) +
      last_member_size
      ) == ret_val;
  }
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = ros2_gopigo3_msg__srv__SPI_Response;
    is_plain =
      (
      offsetof(DataType, data_in) +
      last_member_size
      ) == ret_val;
  }
